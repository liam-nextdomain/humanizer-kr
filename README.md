...
- **`humanizer-kr/SKILL.md`**: 10가지 AI 패턴의 정의, 감지 신호, 스타일 판단 기준, 8단계 처리 워크플로(감지 리포트 → 승인 → 초안 → 재검증 → 최종본), 출력 형식 정의
...
스킬의 구체적인 작업 절차는 **[humanizer-kr/SKILL.md](humanizer-kr/SKILL.md)**를 참고하세요. 워크플로는 다음과 같습니다:

1. **Step 1**: 스타일 판단 (에세이 vs. 논문)
2. **Step 2**: 패턴 감지 및 수량화
3. **Step 2.5**: 감지 결과 리포트 → 사용자 승인 (유형별 선택 가능)
4. **Step 3**: 승인된 유형만 1차 재작성
5. **Step 3.5**: 수정 내용 브리핑
6. **Step 4**: 목소리 협의 (에세이 스타일만)
7. **Step 5**: 남은 문제 재검증 → 사용자 승인
8. **Step 5.5**: 최종 수정 브리핑

핵심 흐름: 감지 후 승인 → 수정 → 브리핑 → 재검증 → 승인 → 최종본
...
- **[humanizer-kr/SKILL.md](humanizer-kr/SKILL.md)** - 10가지 패턴 정의, 감지 신호, 스타일 판단 기준, 8단계 워크플로(승인 게이트 포함), 출력 형식 (마스터 가이드)
