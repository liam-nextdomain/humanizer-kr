...
Academic writing permits enumeration, but AI tends to over-fragment with excessive outline depth (Ⅰ → 1 → 1) → (1) → ①). Apply the following rules:
...
In academic writing, logical connectors are essential — do not strip them wholesale. However, AI overuses example/emphasis type conjunctions (예를 들어, 특히, 예컨대, 다시 말해) while underusing conditional type (한다면, ~라면). Apply the following:
